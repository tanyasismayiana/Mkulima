class Mkulima {
    ...
    removeFarm(farmId) {
      for (let farm in this.farms) {
        if (this.farms[farm].farmId === farmId) {
          this.farms.splice(farm, 1);
          console.log(`Deleted ${farmId}`)
          return `Deleted farm with ID ${farmId} : ${this.farms}`;
        }
      }
      console.log(`Not deleted ${farmId}`)
      return "Nothing Deleted, check id";
    }
  
    updateFarm(farmId, name, farmer, phone, address) {
      let farmFound = this.farms.find((farm) => farm.farmId === farmId);
      if(farmFound !== undefined){
      ...
      return `Farm not found: ${farmFound}`
    }
  ...
  mkulima.removeFarm("3");
  // mkulima.printFarms()
  mkulima.updateFarm("2","Entoyee","Mayiana","0785946367","Kajiado")
  mkulima.printFarms()
  mkulima.getFarm("3")
